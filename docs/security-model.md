...
| Git executable and retained environment | `git` is resolved through inherited `PATH`, and retained global/system configuration controls remain trusted. Repository-local variables, numbered command-config overrides, and discovery controls are removed before worktree validation and lock discovery, so inherited values such as `GIT_DIR`, `GIT_WORK_TREE`, and `GIT_COMMON_DIR` cannot redirect repository paths. |
...
- a general user-configurable name-based ignore or denylist for arbitrary cache, control, and staging paths; `~/.kube/cache` is the only built-in content exclusion, while the runtime-specific private state directory, active worktree, and Git common directory are protected roots;
...
`init` resolves the supplied directory to a canonical absolute path, asks Git to treat it as the worktree root, resolves Git's canonical common administrative directory, and stores the worktree path in local state. The common-directory result is retained for both lock placement and protected-root policy during that repository object's lifetime. The Git subprocess inherits `PATH` and ordinary configuration controls, but `susu` removes repository-local variables, numbered command-config overrides, and discovery controls before validation. Inherited values such as `GIT_DIR`, `GIT_WORK_TREE`, and `GIT_COMMON_DIR` therefore cannot redirect the supplied path or lock placement. Later opens reject a missing path, a path that now resolves through a symlink, a Git root mismatch, or missing storage directories. The binding is path-based: it does not detect a different valid worktree recreated or mounted at the same canonical path.
